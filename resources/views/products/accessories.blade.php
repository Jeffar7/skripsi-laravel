...
@section('title','TokoLokal | Accessories')
			...
			<div class="row">
				...
				<div class="col-sm-3 text-left">
					<form name="sortProducts" id="sortProducts">
						<input type="hidden" name="url" id="url" value="{{ route('accesory') }}">
    ...
    <div class="row">
		<div class="col-3">
			<div class="card px-2 border border-dark" >
				<p class="mb-1 pt-2 font-weight-bold">Accessories Product (<span class="countMen"></span>)</p>
	...
	$(document).ready(function(){
		...
		$("#sort").on('change', function(){
			...
			$.ajax({
				url: url,
				method: 'POST',
				data: {min:min, max:max, category:category, brand:brand, sort:sort, url:url},
		...
		//filter category
		$(".gender, .brand").on('click', function(){
			var min = $("#amount_min").val();
			var max = $("#amount_max").val();
			var category = getIds("gender[]");
			var brand = getIds("brand[]");
		...
		$("#btnUncheckAll").on('click', function(){
			window.location='{{ route("accessory") }}';
		});
	});
</script>
